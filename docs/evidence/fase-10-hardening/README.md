...
encontrados e corrigidos" abaixo. **Confirmado pelo usuário**: o
pipeline real fechou verde (run #10, commit `189df81`, 11m47s) depois
das três correções.
...
| 20  | Pipeline de CI está verde                            | ✅     | `.github/workflows/ci.yml` (lint, formatação, tipos, `npm audit`, testes, build, E2E). Confirmado verde no GitHub Actions real (run #10, commit `189df81`, 11m47s) pelo usuário — este ambiente não tem acesso a `gh`/à API do GitHub para verificar diretamente. Alcançado depois de 3 correções encontradas só pelo pipeline real (ver itens 7 a 9 em "Bugs encontrados"): aviso de `npm audit` publicado no meio do caminho, `AUTH_SECRET` nunca definido no workflow, e uma limitação do WebKit em Linux com o evento de download do navegador |
...
**25/25 critérios verificados.** O item 20 (pipeline de CI) só pôde ser
confirmado com a ajuda do usuário, verificando a execução real no
GitHub Actions — este ambiente não tem acesso a `gh`/à API do GitHub
para checar isso diretamente. Essa verificação real acabou sendo
valiosa além do próprio item 20: revelou 3 problemas genuínos que a
validação local (mesmos comandos, mas ambiente Windows) não conseguia
reproduzir — ver itens 7 a 9 em "Bugs encontrados e corrigidos" abaixo.
...
- [x] Checklist do MVP conferido (25/25 — o item do pipeline de CI foi
      confirmado com a ajuda do usuário, verificando a execução real no
      GitHub Actions)
